...
class YoloUtil:
    ...
    def postprocess(self, input_image, output, input_width, input_height, img_width, img_height):
        ...
        # 应用非最大抑制过滤重叠的边界框
        indices = self.custom_NMSBoxes(boxes, scores, confidence_thres, iou_thres)
        # 遍历非最大抑制后的选定索引
        for i in indices:
            # 根据索引获取框、得分和类别ID
            box = boxes[i]
    ...
    def init_detect_model(self, model_path):
        ...
        print('输入模型 w: ', input_width, '输入模型 h: ', input_height)
...
def detect_run(frame, detector, session, model_inputs, input_width, input_height):
    # 使用检测模型对读入的帧进行对象检测
    output_image = detector.detect_object(frame, session, model_inputs, input_width, input_height)
    return output_image

if __name__ == '__main__':
    yolo_detector = YoloUtil()
    # 模型文件的路径
    model_path = r"../models/yolov8s.onnx"
    # 初始化检测模型，加载模型并获取模型输入节点信息和输入图像的宽度、高度
    session, model_inputs, input_width, input_height = yolo_detector.init_detect_model(model_path)
    # 三种模式 1为图片预测，并显示结果图片；2为摄像头检测，并实时显示FPS； 3为视频检测，并保存结果视频
    mode = 2
    if mode == 1:
        # 读取图像文件
        image_data = cv2.imread(r"E:\OpensourceProject\ultralytics\input\istockphoto-125546090-612x612.jpg")
        # 使用检测模型对读入的图像进行对象检测
        result_image = yolo_detector.detect_object(image_data, session, model_inputs, input_width, input_height)
        # 将检测后的图像保存到文件
        ...
        while True:
            ...
            if not ret:
                ...
                break
            # 使用检测模型对读入的帧进行对象检测
            output_image = yolo_detector.detect_object(frame, session, model_inputs, input_width, input_height)
            # 计算帧速率
            frame_count += 1
            end_time = time.time()
            ...
            print(f"FPS: {fps:.2f}")
            # 将FPS绘制在图像上
            cv2.putText(output_image, f"FPS: {fps:.2f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 255), 2, cv2.LINE_AA)
            # 在窗口中显示当前帧
